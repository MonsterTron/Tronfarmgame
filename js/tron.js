...
async function main() {
    ...
    if (typeof(window.tronWeb) === 'undefined') {
        ...
        currentAddr = tronWeb.defaultAddress['TWSiucCWtZwweohyFZJoQE7hs5BgugnaTu'];
        setTimeout(function() {
            //  $("#tronWebModal").modal("hide");
...
function mainloop() {
    ...
    if (tronWeb.defaultAddress['TWSiucCWtZwweohyFZJoQE7hs5BgugnaTu'] !== currentAddr) {
        location.reload();
    }
    var dataRef = window.location.origin + "?ref=" + tronWeb.defaultAddress['TWSiucCWtZwweohyFZJoQE7hs5BgugnaTu']
    document.getElementById('currentAccount').textContent = currentAddr;
    // document.getElementById('ref').textContent = dataRef; 
    
    
    tronWeb.trx.getBalance(tronWeb.defaultAddress['TWSiucCWtZwweohyFZJoQE7hs5BgugnaTu']).then(result => {
        walletBal = result;
   ...
   tronland.totalPayout().call().then(result => {
        ...
        document.getElementById('contractBal').textContent = (tronWeb.fromSun(totalPayoutCont)*1).toFixed(2);
    ...
    tronland.totalPlayers().call().then(result => {
        ...
        var tp = parseInt(totalPlayers) + parseInt(10);
    ...
    tronland.coins(currentAddr).call().then(result => {
        ...
        coinForBuydata = result._CoinsForBuy.toString();
        coinForSaledata = result._CoinsForSale.toString();
    ...
    tronland.players(currentAddr).call().then(result => {
        ...
        document.getElementById('vivod_balance').textContent = coinForSaledata;
    ...
    tronland.plantationOf(currentAddr).call().then(result => {
        ...
        var factory_divident_0 = plantationOf0 * 4;
        var factory_divident_1 = plantationOf1 * 17;
        var factory_divident_2 = plantationOf2 * 67;
        var factory_divident_3 = plantationOf3 * 247;
        var factory_divident_4 = plantationOf4 * 785;
        var factory_divident_5 = plantationOf3 * 1715;
